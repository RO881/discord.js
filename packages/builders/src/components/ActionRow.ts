...
export class ActionRowBuilder extends ComponentBuilder<APIActionRowComponent<APIActionRowComponentTypes>> {
	 ...
	 * @param input - The buttons to add
	 */
	public addSKUIDButtonComponents(
		...input: RestOrArray<
			APIButtonComponentWithSKUId | SKUIdButtonBuilder | ((builder: SKUIdButtonBuilder) => SKUIdButtonBuilder)
		>
	): this {
		const normalized = normalizeArray(input);
		const resolved = normalized.map((component) => resolveBuilder(component, SKUIdButtonBuilder));
	 ...
	 * @param input - The buttons to add
	 */
	public addURLButtonComponents(
		...input: RestOrArray<
			APIButtonComponentWithURL | URLButtonBuilder | ((builder: URLButtonBuilder) => URLButtonBuilder)
		>
	): this {
		const normalized = normalizeArray(input);
		const resolved = normalized.map((component) => resolveBuilder(component, URLButtonBuilder));
